...
JISのCaps Lock／英数と半角／全角を使う[操作端末用IME補助](docs/moonlight-ime-client.md)を追加しました。物理キーを押して離した時だけ短い信号を送り、ホスト側で1回のIME切り替えにします。長押しや解放欠落では連続切り替えを行わず、物理HHKBの設定は変更しません。初期状態では無効で、両PCへの導入が必要です。自動検証は完了していますが、操作端末との接続試験は未完了です。

v2では、クライアントで押下だけ増えて解放が0になる場合に備え、Moonlightの入力配列を確認して英語（US）入力を選択します。操作端末で初回は配布ZIPの `Start-MoonlightImeClient.cmd` を実行してください。日本語入力を保持したままUS入力とアプリごとの入力設定を準備します。

旧版のIMEキー直接置換は撤回済みです。以前の設定を使っている場合の[解除と暫定リピート停止](docs/setup.md#us配列ホストとjis配列クライアントのime切り替え)も参照してください。
...
| `src/MoonlightImeClient/`、`src/KeyboardBridge.Shared/` | 操作端末用IME補助、押下・解放ポリシーとWin32入力 |
