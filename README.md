...
US配列のHHKBを使うホストへJIS配列のMoonlight端末から接続し、「半角／全角」でバッククォートが入力される場合は、[リモート用のキー割り当て](docs/setup.md#us配列ホストとjis配列クライアントのime切り替え)を設定できます。Caps Lockへの割り当ても設定できますが、JISの「英数」の押下・解放が正しく届かない場合はクライアント側の調整が必要です。物理キーボードの配列を保持し、キーごとに補正の有効・無効を切り替えられます。
