...
旧版のIMEキー補正には、キー解放の通知が欠落すると半角・全角が連続して切り替わる問題があり、直接置換の機能を撤回しました。既に設定した場合は[補正の解除手順](docs/setup.md#us配列ホストとjis配列クライアントのime切り替え)を実行してください。解除後も記号が連続入力される場合の暫定的なリピート停止と、その制限も同ページに記載しています。物理HHKBの設定は変更しません。JISのCaps Lock／英数単押しによるリモートIME切り替えは、現在未対応です。
