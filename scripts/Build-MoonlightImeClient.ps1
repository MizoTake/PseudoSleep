...
& $taskCompiler /nologo /target:winexe /platform:anycpu /optimize+ /warnaserror+ "/out:$taskClient" /reference:System.Windows.Forms.dll /reference:System.Drawing.dll /reference:System.Xml.dll "/win32manifest:$(Join-Path $taskRoot 'src\PseudoSleep\app.manifest')" (Join-Path $taskRoot 'src\MoonlightImeClient\Program.cs') (Join-Path $taskRoot 'src\MoonlightImeClient\ClientKeyboardLayout.cs') (Join-Path $taskRoot 'src\KeyboardBridge.Shared\ClientLayoutGuard.cs') (Join-Path $taskRoot 'src\KeyboardBridge.Shared\KeyGates.cs') (Join-Path $taskRoot 'src\KeyboardBridge.Shared\KeyboardNative.cs')
...
foreach ($taskScript in @('Set-MoonlightClientKeyboard.ps1', 'Start-MoonlightImeClient.ps1', 'Start-MoonlightImeClient.cmd')) { Copy-Item -LiteralPath (Join-Path $PSScriptRoot $taskScript) -Destination $taskOutput }
$taskFiles = @('MoonlightImeClient.exe', 'README.md', 'LICENSE', 'Set-MoonlightClientKeyboard.ps1', 'Start-MoonlightImeClient.ps1', 'Start-MoonlightImeClient.cmd') | ForEach-Object { Join-Path $taskOutput $_ }
